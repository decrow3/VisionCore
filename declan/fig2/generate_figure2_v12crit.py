...
"""
Figure 2 — v12crit variant: same decomposition as generate_figure2.py but with
two additional per-neuron inclusion criteria borrowed from test_rowley12.py:

  1. Split-half PSTH reliability (r² >= MIN_RELIABILITY = 0.025, 20 splits)
  2. NaN-fraction gate: neurons with > MAX_NAN_FRAC (20%) missing fixrsvp
     time-bins are excluded (mirrors test_rowley12 Pool B gate)

For Rowley sessions, the script expands the YAML `cids` to sorter
contamination-passing V1 units when available and applies the dots RF SNR
visual criterion from the Rowley dots calibration outputs. This avoids relying
on Gaborium STA quality for Luke.
...
# v12crit extra inclusion criteria (mirrors test_rowley12.py Pool A/B logic)
...
DATASET_CONFIGS_PATH = VISIONCORE_ROOT / "experiments" / "dataset_configs" / "multi_basic_120_long_yates_rowley.yaml"
...
# Output directories
FIG_DIR = FIGURES_DIR / "fig2_v12crit"
STAT_DIR = STATS_DIR / "fig2_v12crit"
...
def _resolve_session_data_root(dataset_directory):
    """Walk up from the dataset dir to find the folder containing dpi_calibration."""
...
def _load_dots_snr_cache(session_data_root, eye):
    """Load primary-eye dots RF SNR for Rowley units.
    ...
    Returns (cids, max_snr) arrays, or (None, None) if neither source is present.
    """
...
def _split_half_reliability(robs_trials, n_splits, seed=42,
                            min_valid_bins=10, min_trials_per_half=2):
    """Mean split-half PSTH r² across random draws. Ported from test_rowley12."""
    ...
    for _ in range(n_splits):
        ...
        with warnings.catch_warnings():
            ...
            psth_a = np.nanmean(robs_trials[idx_a], axis=0)  # (T, n_units)
            psth_b = np.nanmean(robs_trials[idx_b], axis=0)
        cnt_a = np.sum(np.isfinite(robs_trials[idx_a]), axis=0)
        cnt_b = np.sum(np.isfinite(robs_trials[idx_b]), axis=0)
        for u in range(n_units):
            a, b = psth_a[:, u], psth_b[:, u]
            fin = (np.isfinite(a) & np.isfinite(b)
                   & (cnt_a[:, u] >= min_trials_per_half)
                   & (cnt_b[:, u] >= min_trials_per_half))
            if fin.sum() < min_valid_bins:
                continue
            if np.std(a[fin]) <= 0 or np.std(b[fin]) <= 0:
                continue
            r2_accum[u] += np.corrcoef(a[fin], b[fin])[0, 1] ** 2
            r2_count[u]  += 1
    with np.errstate(invalid="ignore", divide="ignore"):
        return np.divide(r2_accum, r2_count,
                         out=np.zeros_like(r2_accum), where=r2_count > 0)
...
cache_path = CACHE_DIR / "fig2_decomposition_v12crit_yates_rowley.pkl"
...
if cache_path.exists() and not RECOMPUTE:
    ...
    for cfg in dataset_configs:
        ...
        # v12crit: for Rowley sessions expand cids to sortercontam when present
        # so a sorter-quality V1 pool is loaded; dots RF SNR is then applied
        # in-code as Criterion 1 (matching test_rowley12 dots_rf mode) rather
        # than relying on the YAML visual list.
        if is_rowley:
            all_v1_cids, rowley_pool_source = _rowley_initial_cid_pool(cfg)
            if all_v1_cids.size == 0:
                print("  Skipping: Rowley config has no sortercontam/qccontam/cids")
                continue
            print(
        ...
        if is_rowley:
            ...
            if snr_cids is not None:
                ...
                print(
                    "  Skipping: dots RF SNR not found at "
        ...
        # NaN fraction must be computed within valid (within-fixation) bins only;
        # align_fixrsvp_trials pads short fixations with NaN out to valid_time_bins,
        # so a naive mean would inflate nan_frac massively for all neurons.
        valid_bins = valid_mask[:, :, None]  # (n_trials, T, 1) broadcast over neurons
        n_valid = float(valid_bins.sum())
        nan_frac = (np.isnan(robs) & valid_bins).sum(axis=(0, 1)) / max(n_valid, 1.0)
        nan_ok = nan_frac <= MAX_NAN_FRAC
        reliability = _split_half_reliability(robs, N_REL_SPLITS, seed=42)
        rel_ok = reliability >= MIN_RELIABILITY
        v12_mask = nan_ok & rel_ok
        n_pre  = robs.shape[2]
        robs   = robs[:, :, v12_mask]
        n_post = robs.shape[2]
        print(f"  Quality gates (NaN <= {MAX_NAN_FRAC}, r² >= {MIN_RELIABILITY}): "
              f"{n_post}/{n_pre} neurons pass")
        if n_post < 3:
            print(f"  Skipping: too few neurons after v12crit filters")
            continue
