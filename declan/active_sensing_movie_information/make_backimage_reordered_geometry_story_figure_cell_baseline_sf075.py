...
OUT_STEM = "backimage_real_trace_geometry_reordered_story_figure_cell_baseline_sf075"
SF_METRIC_COL = "sf_split_metric"
LOW_SF_MAX_CPD = 0.50
HIGH_SF_MIN_CPD = 0.75
CONTOUR_COHERENCE_MIN = 0.50
MIN_OSI = 0.05
MATCH_MAX_DEG = 22.5
...
def _plot_figure(panel_b: pd.DataFrame, component: pd.DataFrame) -> plt.Figure:
    ...
    fig.text(
        ...
        0.035,
        (
            "B uses strong-contour image windows (coherence >= 0.50), SF < 0.50 as low, and SF >= 0.75 as high; "
            "units with 0.50 <= SF < 0.75 are excluded. C-E use SF >= 0.75 and omit the full-trajectory curve."
        ),
        ha="center",
        va="bottom",
        fontsize=8.5,
