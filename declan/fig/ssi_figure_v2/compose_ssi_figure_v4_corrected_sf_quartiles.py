...
def build_inputs() -> dict[str, Any]:
    ...
    if manifest["n_complete_rounds"] != 12 or len(condition) != 12000:
        raise ValueError("Expected the frozen 12-round/12,000-condition assembly")
...
def compute_path_tables(data: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, np.ndarray]]:
    ...
    for relation, masks in (("strong_contours", strong_masks), ("contour_matched", matched_masks)):
        for group in GROUPS:
            ...
            if relation == "contour_matched" and group == "sf_q4":
...
def draw_path_panel(table: pd.DataFrame, *, relation: str, out_path: Path, figsize: tuple[float, float], label: str, title: str) -> None:
    ...
    fig.text(
        0.12, 0.895, "corrected cache · 12 complete balanced rounds (interim)",
        ha="left", va="top", fontsize=5.7, color="0.38",
    )
    for group in GROUPS:
        for context_name, filled, linestyle in (("drift_only", False, "-"), ("microsaccade", True, "--")):
            ...
            ax.errorbar(
                sub.x_median, sub.delta_percent, yerr=yerr, color=COLORS[group], marker="o",
                mfc=COLORS[group] if filled else "white", mec=COLORS[group], ms=3.5,
                lw=1.3, ls=linestyle, capsize=1.5,
    ...
    ax.text(0.98, 0.02, "open: drift only\nfilled/dashed: microsaccade", transform=ax.transAxes, ha="right", va="bottom", fontsize=5.3)
...
def draw_component_panel(table: pd.DataFrame, *, out_path: Path, figsize: tuple[float, float]) -> None:
    ...
    panel_header(fig, "E", "Across-contour spread limits\nhighest-SF-quartile benefit")
    ...
    for component in ("across", "along"):
        ...
        yerr = np.vstack([sub.delta_percent - sub.ci95_low, sub.ci95_high - sub.delta_percent])
        ax.errorbar(
            sub.x_median, sub.delta_percent, yerr=yerr, color=color, ls=linestyle,
...
def main() -> None:
    ...
    draw_path_panel(
        ...
        label="D",
        title="Contour alignment reshapes\nSF dependence",
    )
    ...
    manifest = {
        ...
        "scope": (
            "Population panels use 12 complete balanced corrected-cache rounds (12,000 movies; "
            "interim, not a complete half-bank/full-bank result). Explanatory schematics A/C are unchanged."
        ),
        "quartiles": "61 recorded-validated units split 16/15/15/15 after the r >= 0.5 gate",
