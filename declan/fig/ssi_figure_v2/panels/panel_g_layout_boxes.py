#!/usr/bin/env python3
"""Editable box template for Panel G's block layout (reference crop, zoomed
aperture, and the across/along RMS-excursion explainer): export the current
...
as an override that generate_ssi_figure_v2.draw_contour_components_panel
reads instead of its own hardcoded defaults (panel_g_default_layout_boxes()).

Same pattern as panel_a_layout_boxes.py (see that module's docstring for
the general rationale). crop/zoom's widths are derived from height to
preserve the real image's own aspect ratio (data_width_for_physical_aspect)
-- an override is used verbatim regardless, so an aspect that doesn't
match the source image will letterbox/crop within the box rather than
stretch it (draw_plain_crop/add_zoomed_crop_view already lock aspect
internally).

Round trip:
    uv run python declan/fig/ssi_figure_v2/panels/panel_g_layout_boxes.py export
    # open outputs/fig/ssi_figure_v2/panels/cache/panel_g_layout_boxes.svg,
    # drag/resize crop / zoom / rms_explainer, save
    uv run python declan/fig/ssi_figure_v2/panels/panel_g_layout_boxes.py import
    # panel_g_local_contour_detail.build_panel() now reflects the edit
"""
...
from panels import panel_g_local_contour_detail as panel_g  # noqa: E402
from panels.svg_box_utils import find_rect_bbox  # noqa: E402

CACHE_DIR = ROOT / "outputs" / "fig" / "ssi_figure_v2" / "panels" / "cache"
BOXES_SVG = CACHE_DIR / "panel_g_layout_boxes.svg"
OVERRIDES_JSON = panel_g.LAYOUT_OVERRIDES_JSON

BOX_IDS = ["crop", "zoom", "rms_explainer"]
BOX_COLORS = {
    "crop": "#0072B2",
    "zoom": "#CC79A7",
    "rms_explainer": "#009E73",
}
...
def _box_to_svg_rect(box: tuple[float, float, float, float], figsize: tuple[float, float]) -> tuple[float, float, float, float]:
    x, y, w, h = box
    x0_pt, y0_pt = panel_g.data_frac_to_page_pt(x, y, figsize)
    x1_pt, y1_pt = panel_g.data_frac_to_page_pt(x + w, y + h, figsize)
...
def _svg_rect_to_box(
    ...
    x0_frac, y0_frac = panel_g.page_pt_to_data_frac(x0_pt, y_bottom_bottom_up, figsize)
    x1_frac, y1_frac = panel_g.page_pt_to_data_frac(x1_pt, y_top_bottom_up, figsize)
    return x0_frac, y0_frac, x1_frac - x0_frac, y1_frac - y0_frac


def export_layout_boxes(out_svg: Path = BOXES_SVG) -> Path:
    figsize = panel_g.DEFAULT_FIGSIZE
    panel_pdf = panel_g.build_panel()  # fresh render, reflects any existing overrides
    boxes = panel_g.compute_current_boxes(figsize)
    ...
    lines = [
        ...
        "  <title>Panel G editable block-layout boxes</title>",
        (
            "  <desc>Background is the current render, faded. Drag/resize "
            "crop / zoom / rms_explainer, save, then run: "
            "panels/panel_g_layout_boxes.py import</desc>"
        ),
        (
...
def main() -> None:
    if len(sys.argv) < 2 or sys.argv[1] not in {"export", "import"}:
        print("usage: panel_g_layout_boxes.py {export|import}")
        raise SystemExit(2)
    if sys.argv[1] == "export":
        path = export_layout_boxes()
        print(f"wrote {path}")
        print(f"Drag/resize any of: {', '.join(BOX_IDS)}, save, then run:")
        print("  uv run python declan/fig/ssi_figure_v2/panels/panel_g_layout_boxes.py import")
    else:
