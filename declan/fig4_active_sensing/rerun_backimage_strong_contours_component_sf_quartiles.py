#!/usr/bin/env python3
"""Checkpoint 10: page-19 strong-contour component SSI with new SF quartiles."""
...
DEFAULT_OUT = ROOT / (
    ...
    "checkpoint_10_strong_contours_component_sf_quartiles"
)
...
def main() -> None:
    ...
    if not validation["passed"]:
        raise ValueError(f"Historical page-19 reconstruction failed: {validation}")
    ...
    schematic.plot_component_population_12_panel(
        summary,
        args.out_dir,
        stem="019_strong_contours_valid_fit_sf_quartiles_across_along_components",
        title="Spike-weighted population SSI - strong contours: across and along components",
    ...
    make_direct_overlay(
        summary,
        args.out_dir,
        stem="checkpoint_10_across_vs_along_direct_overlay",
        title="Strong-contour images: across- versus along-contour population modulation",
    )
    ...
    manifest = {
        ...
        "source_test": "PDF page 19: strong-contour across/along component-path population SSI",
        "replacement_scope": "53 strong-contour image windows and all 85 model-valid units in tie-aware preferred-SF quartiles; no OSI gate",
        ...
        "selected_unit_image_counts": {str(k): int(v) for k, v in image_counts.items()},
        "sources": {
        ...
        "not_run": "No later orientation-conditioned or final Figure 4 analysis was regenerated.",
    }
    ...
    readme = f"""# Checkpoint 10: strong-contour component SF quartiles

This regenerates page 19 on the same strong-contour image windows as checkpoint
9, using the new preferred-SF quartiles. Across and along components are defined
relative to each image's contour axis and binned independently.
...
`historical_contract_validation.json`. No later orientation-conditioned analysis
was run here.
"""
