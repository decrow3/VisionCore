#!/usr/bin/env python3
"""Checkpoint 13: page-22 intermediate-orientation population SSI with new SF quartiles."""
...
DEFAULT_OUT = ROOT / (
    ...
    "checkpoint_13_contour_intermediate_population_sf_quartiles"
)
...
def main() -> None:
    ...
    if not validation["passed"]:
        raise ValueError(f"Historical page-22 reconstruction failed: {validation}")
    ...
    schematic.plot_sf_rows_population_panel(
        ...
        stem="022_contour_intermediate_valid_fit_sf_quartiles_population_absolute_delta",
        title="Spike-weighted population SSI - strong contours, intermediate-orientation units",
        subtitle=(
            "new SF quartiles; OSI >= 0.05 and 22.5 deg < unit-contour mismatch < 67.5 deg; "
    ...
    make_unit_weighting_figure(
        ...
        title="Population weighting audit: strong contours and intermediate-orientation units",
        stem="checkpoint_13_population_vs_equal_unit_audit",
    )
    ...
    manifest = {
        ...
        "status": "checkpoint_13_complete",
        "source_test": "PDF page 22 / figure label 022: intermediate-orientation strong-contour population SSI",
        "replacement_scope": "new tie-aware SF quartiles among 85 valid parametric fits; historical orientation relation retained",
        "selection_contract": "strong contour images; unit OSI >= 0.05; 22.5 < axial unit-contour mismatch < 67.5 degrees; disjoint from aligned and orthogonal gates",
        ...
        "downstream_revision_boundary": "later Figure 4 analyses use cell-matched baselines, fixed SF thresholds, contour coherence >= 0.20, and revised component binning; those changes are not mixed into this checkpoint",
        ...
        "not_run": "No intermediate-orientation component decomposition (page 23) or later revised Figure 4 panel was regenerated.",
    }
    ...
    readme = f"""# Checkpoint 13: intermediate-orientation population SF quartiles

This regenerates historical figure label 022 with the new preferred-SF
quartiles. The intermediate relation is disjoint from the aligned and orthogonal
gates and is defined relative to each local contour, not the screen axes.
...
cell-matched Figure 4 revisions remain separate.
"""
