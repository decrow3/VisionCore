...
- **Panel** `declan/fig3/generate_fig3d.py::plot_panel_d` — scatter of model vs
  empirical 1−α; analytic primary, split-half reported as agreement check. Full
  logic in the module docstring. NOT yet integrated into `generate_figure3.py`.
- **Diagnostics** `declan/fig3/diagnose_fig3d_alpha.py` (cache-only) and
  `declan/fig3/simulate_fig3d_control.py` (Poisson-simulation pipeline control —
  PRELIMINARY/confounded, see below).

## Cache change (already applied + verified)

`_fig3_data.py::_run_inference` now stores per session `eyepos_used` (n_trials,
...
`simulate_fig3d_control.py` Poisson-samples the model rates and runs the *identical*
`run_covariance_decomposition` (1-bin, below_threshold 0.05°). On **fig3-aligned**
data this does **not** reproduce fig2 (obs_pipeline ~0.476 vs fig2 ~0.725), so it is
confounded — the fig3 alignment differs from fig2's `align_fixrsvp_trials`. Localized
...
```bash
cd VisionCore/declan/fig3
uv run python generate_fig3d.py            # panel + per-subject stats
uv run python diagnose_fig3d_alpha.py      # cache-only floor/cloud/Logan diagnostics
uv run python simulate_fig3d_control.py --n-sim 5   # PRELIMINARY control (~slow, GPU)
