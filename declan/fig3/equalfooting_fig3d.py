...
four ways, all in the SAME fig3 frame:
...
Decisive comparisons (good cells):
  ...
  * B_obs    vs fig2   -- fig3-frame pipeline vs fig2-frame pipeline (alignment).
  * A_shared vs A_panel-- does the valid-mask choice (eye-finite vs dfs!=0) matter?

Usage:
    uv run python equalfooting_fig3d.py [--recompute]
"""
import argparse
import numpy as np
import dill
...
def make_figure(res):
    ...
    pairs = [
        ...
        ("fig2", "B_obs", "fig2 (empirical)", "neuron pipeline (B, fig3 frame)"),
    ]
