"""Side-by-side viewer for the Fig 3 bottom-row ablation (zeroed vs permuted).
...
its own. The canonical pipeline is `_fig3_ablation_data.py` (data) +
`generate_fig3{h,i,j}.py` (panels), assembled by `generate_figure3.py`.
...
from _fig3_ablation_data import load_ablation_data, print_ablation_stats, ABLATIONS
...
def main(recompute=False):
    ...
    fig.suptitle("Fig 3 bottom row — extraretinal ablation "
