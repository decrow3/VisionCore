"""Run the Figure 4C known-start continuous-joint observer candidate.

This wrapper records the cleaner post-affine candidate: the same native
scale-conditioned compact quadratic observation model as the promoted
inferred-start observer, but with a soft prior on the measured initial eye
position. It still hides the trajectory after the first sample.
"""
...
DEFAULT_OUT_DIR = SOURCE_ROOT / "continuous_joint_quadratic_poisson_scale_conditioned_knownstart_full"
DEFAULT_CALIBRATED_OUT_DIR = (
    SOURCE_ROOT / "continuous_joint_quadratic_poisson_scale_conditioned_knownstart_calibrated_full"
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out-dir", type=Path, default=DEFAULT_CALIBRATED_OUT_DIR)
    parser.add_argument(
        "--max-tables",
        type=int,
        default=0,
...
def analyzer_argv(args: argparse.Namespace) -> list[str]:
    argv = [*KNOWNSTART_ANALYZER_ARGS, "--out-dir", str(Path(args.out_dir))]
