...
The next goal is continuous feature-embedding reconstruction: infer a compact
feature vector directly from the response movie instead of computing a
posterior-weighted average over candidate images.
...
The pre-feature-embedding version has been saved here:
...
The continuous feature-embedding branch changes only the target/inference
contract:

```text
feature embedding z = whitened PCA(phi(I))
response model      = A z + noise
feature prior       = z ~ N(0, I)
feature estimate    = E[z | response]
feature score       = cosine(z_hat, z_true)
```

This removes finite candidate selection at test time. The current
implementation still learns the empirical feature prior and response map from
the finite BackImage cache, and the hidden-eye first pass treats sampled
trajectories as nuisance variation rather than optimizing a latent trajectory
jointly with `z`.
...
Implemented continuous feature-embedding reconstruction branch:

```text
declan/figure4_active_sensing_atlas/scripts/build_panel_c_continuous_feature_embedding_reconstruction.py
```

Implemented continuous-tau MLP feature decoder:

```text
declan/figure4_active_sensing_atlas/scripts/build_panel_c_continuous_tau_mlp_feature_decoder.py
```
...
Continuous feature-embedding outputs:
...
Continuous-tau MLP feature-decoder outputs:
...
continuous-eye estimator and the nonlinear feature-embedding upper-bound. It no
longer selects an endpoint image candidate. Instead, it trains MLP readouts to
map compact response movies, optionally concatenated with continuous `tau`
features, directly to `fold_zscore_whitened_pca` coordinates of
`pyramid_local_field`.

Full-cache all-scale feature cosine:
...
![continuous feature-embedding reconstruction](figures/panel_C/diagnostics/continuous_feature_embedding/continuous_feature_embedding_reconstruction.png)
...
The new continuous feature-embedding reconstruction branch is a next-step
diagnostic, not the promoted 4C replacement. It uses a 32D whitened
`pyramid_local_field` feature embedding and a 20D image-disjoint compact
...
using trajectory-nuisance training alone.

Feature-embedding artifact:
...
## Continuous Feature-Embedding Option Sweep

The candidate-free feature branch now has a small target-space sweep in:
...
The default sweep keeps the same compact response basis and response-table
cache, but compares four 32D `pyramid_local_field` target spaces:
...
The full-cache all-scale result is:
...
The fold z-scored whitened target is the preferred next option to attempt
because it is closer to the 4B target preprocessing while preserving the
Gaussian-prior interpretation of `z`. The unwhitened option is useful as a
scale/control diagnostic, but it should not be treated as the clean Gaussian
posterior endpoint.

## Nonlinear MLP Upper-Bound Attempt

The next hammer pass adds a Tejas-style ReLU MLP decoder to the same continuous
feature-embedding script. This is explicitly an information upper-bound probe:
it does not ask whether a simple downstream linear readout exists. It asks
whether the compact response movie contains enough information for a strong
nonlinear decoder to recover the compact feature embedding.
...
```text
decoder: Tejas-style ReLU MLP
target: fold_zscore_whitened_pca, 32D pyramid_local_field
response: 20D image-disjoint compact basis over the response movie
hidden units: 512
layers: 4
dropout: 0
optimizer: AdamW, lr=1e-3, weight_decay=1e-5
split: source-row cross-fit with source-group validation inside each training fold
```
...
hidden moving - 0x stabilized:  +0.0662  CI [+0.0450, +0.0887]
known moving - 0x stabilized:   -0.0491  CI [-0.0684, -0.0302]
```
...
```text
The compact moving response contains feature information above the stabilized
counterfactual when decoded with a strong nonlinear readout. This is not a
linear-decodability claim. The known-eye row underperforms hidden-eye, probably
because the hidden/tau-nuisance bank has many trajectory draws per image and
therefore acts as augmentation, while the known-eye bank has one trajectory per
candidate.
```
