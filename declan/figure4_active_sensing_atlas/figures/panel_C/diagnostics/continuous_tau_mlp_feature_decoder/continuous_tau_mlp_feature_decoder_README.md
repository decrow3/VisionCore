...
+ response-by-tau interaction features
-> z_hat ~= PCA(phi(image))
```
...
continuous tau + interactions:         0.1124
true tau + interactions:               0.0743
0x stabilized response:                0.2146
known-eye model response:               0.1467
```

Primary paired contrast:

```text
continuous tau - 0x stabilized: -0.1022  CI [-0.1212, -0.0834]
```
