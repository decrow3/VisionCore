...
This document speaks to the local mechanistic branch of the 4B claim rather
than the main aggregate panel by itself. It asks whether the exact image-trace
pairing strengthens the motion-induced feature readout, which would make the
4B title more mechanistic. If the paired-trace evidence weakens, the main 4B
claim should remain an aggregate, pose-known feature-decoding result rather
than a claim about image-specific behavioral pairing. The motivation,
assumptions, estimator contract, controls, and caveats below all serve that
adjudication.
...
The local 4B branch asks whether the actual image and the actual trace recorded
with that image work better together than matched but unpaired combinations.
This is a stricter question than the aggregate panel. The aggregate panel asks
whether empirical-like motion helps as a distribution; the local branch asks
whether the particular pairing carries extra local information.
...
The decoded target is the local `pyramid_local_field` feature target with
`k = 16`. The decoder uses grouped-by-image cross-validation, so the train/test
split is made by image rather than by repeated samples from the same image. The
score is the feature-decoding gain from adding the motion-induced response
summary to the static response. The pairing contrast subtracts the gain for a
control trajectory assignment from the gain for the actual paired trace.
