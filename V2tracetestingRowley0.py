...
# Load spike locations for each cluster and cluster IDs
...
spike_samples_imec0 = np.load(imec0_path + '/spike_times.npy') #in imec0 samples
spike_samples_imec1 = np.load(imec1_path + '/spike_times.npy') #in imec1 samples

params_imec0 = np.loadtxt(imec0_path + '/params.py', dtype='str')
params_imec1 = np.loadtxt(imec1_path + '/params.py', dtype='str')

sampling_rate_imec0 = float(params_imec0[np.where(params_imec0=='sample_rate')[0][0]][2])
sampling_rate_imec1 = float(params_imec1[np.where(params_imec1=='sample_rate')[0][0]][2])

from scipy.io import loadmat
# Convert spike samples to nidaq samples, via interpolation
NidaqImec0timing = loadmat(datadir + 'Luke0804_V2V1_NidaqImec0timing.mat')
NidaqImec1timing = loadmat(datadir + 'Luke0804_V2V1_NidaqImec1timing.mat')

Imec0times=NidaqImec0timing['NidaqImec0timing']['imec0Time'][0][0]
Nidaq4Imec0times=NidaqImec0timing['NidaqImec0timing']['NidaqTime'][0][0]
Imec1times=NidaqImec1timing['NidaqImec1timing']['imec1Time'][0][0]
Nidaq4Imec1times=NidaqImec1timing['NidaqImec1timing']['NidaqTime'][0][0]

# Interpolate spikes in samples to nidaq samples
spike_Nidaq_samples_imec0 = np.interp(spike_samples_imec0, Imec0times.flatten(), Nidaq4Imec0times.flatten())
spike_Nidaq_samples_imec1 = np.interp(spike_samples_imec1, Imec1times.flatten(), Nidaq4Imec1times.flatten())

# Convert to time in seconds with the nidaq sampling rate
meta_file_path = (datadir +'Luke0804_V2V1_g0_t0.nidq.meta')
metadata = {}
...
NidaqSamplingRate = float(metadata['niSampRate'])

spike_times_imec0_ts = spike_Nidaq_samples_imec0 / NidaqSamplingRate
spike_times_imec1_ts = spike_Nidaq_samples_imec1 / NidaqSamplingRate

spike_cids_imec0=np.unique(spike_clusters_imec0)
spike_cids_imec1=np.unique(spike_clusters_imec1)

#%% Remake ptb2ephys = sess.load_ptb2ephys()[0]
#  load up Luke0804_V2V1_NidaqPTBtiming.mat for ptb2ephys function fit a linear function
NidaqPTBtiming = loadmat(datadir + 'Luke0804_V2V1_NidaqPTBtiming.mat')
PTBtimes=NidaqPTBtiming['NidaqPTBtiming']['PTBTime'][0][0]
Nidaq4PTBtimes=NidaqPTBtiming['NidaqPTBtiming']['NidaqTime'][0][0]

#plot to check linearity
plt.figure()
plt.plot(Nidaq4PTBtimes.reshape(-1, 1), PTBtimes.reshape(-1, 1), 'k.')

#fit linear function, RANSAC to be robust to outliers
from sklearn.linear_model import RANSACRegressor, LinearRegression
model = RANSACRegressor(LinearRegression())
model.fit(PTBtimes.reshape(-1, 1), Nidaq4PTBtimes.reshape(-1, 1))
_coef = float(np.ravel(model.estimator_.coef_)[0])
_inter = float(np.ravel(model.estimator_.intercept_)[0])
def ptb2ephys(ptb_times):
    # Vectorized affine transform; accepts scalars and arrays
    arr = np.atleast_1d(ptb_times)
    try:
        arr = arr.astype(float)
    except Exception:
        arr = np.asarray(arr, dtype=np.float64)
    out = _coef * arr + _inter
    return float(out[0]) if np.isscalar(ptb_times) or np.ndim(ptb_times) == 0 else out

# Test ptb2ephys function converting PTBtimes
ptb_test_times = PTBtimes
ptb2ephys_test_times = ptb2ephys(ptb_test_times)
plt.figure()
plt.plot(PTBtimes, np.transpose(Nidaq4PTBtimes), 'k.')
plt.plot(ptb_test_times, ptb2ephys_test_times, 'ro')
plt.xlabel('Ephys time (s)')
plt.ylabel('PTB time (s)')

#%% Compute mean position for each cluster
mean_pos_imec0 = np.nan*np.zeros((np.unique(spike_clusters_imec0).max()+1, 1))
mean_pos_imec1 = np.nan*np.zeros((np.unique(spike_clusters_imec1).max()+1, 1))

for cc in range(np.unique(spike_clusters_imec0).max()+1):
    cc_pos = spike_positions_imec0[spike_clusters_imec0 == cc]
    if len(cc_pos) == 0:
        continue
    mean_pos_imec0[cc] = np.mean(np.mean(cc_pos, axis=0))
    print(f'Cluster {cc}: Mean position {mean_pos_imec0[cc]}')

for cc in range(np.unique(spike_clusters_imec1).max()+1):
    cc_pos = spike_positions_imec1[spike_clusters_imec1 == cc]
    if len(cc_pos) == 0:
        continue
    mean_pos_imec1[cc] = np.mean(np.mean(cc_pos, axis=0))
    print(f'Cluster {cc}: Mean position {mean_pos_imec1[cc]}')

#%% Plot mean positions
plt.figure()
plt.plot(spike_cids_imec0, mean_pos_imec0, 'bo', label='imec0 (V2)')
plt.plot(np.max(spike_cids_imec0)+spike_cids_imec1+1, mean_pos_imec1,'ro', label='imec1 (V1)')
plt.legend()
# plot a horizontal line at at 1250 to separate shallow and deep neurons
plt.axhline(1250, color='k', linestyle='--')
plt.ylabel('Distance from tip (um)')
plt.xlabel('Cluster ID')

V1_neurons = np.max(spike_cids_imec0) + 1 + spike_cids_imec1[mean_pos_imec1[:,0] < 1250]
V2_neurons = spike_cids_imec0[mean_pos_imec0[:,0] >= 1250]

#%%
spikes=np.concatenate([spike_times_imec1_ts, spike_times_imec0_ts])
clu=np.concatenate([spike_clusters_imec1 + np.max(spike_cids_imec0) + 1, spike_clusters_imec0])
st=spikes
...
#%% Raster all spikes with shaded boxes for trials, colored by stimulus type
num_trials = len(exp['D'])
trial_protocols = [exp['D'][i]['PR']['name'] for i in range(num_trials)]

unique_protocols = np.unique(trial_protocols)
protocol_colors = plt.cm.get_cmap('tab10', len(unique_protocols))
plt.figure(figsize=(12, 6))
#every 10th spike for visibility
plot_raster(st[::10], clu[::10], height=0.8)
for i in range(num_trials):
    protocol = trial_protocols[i]
    color = protocol_colors(np.where(unique_protocols == protocol)[0][0])
    # Use D[i]['STARTCLOCKTIME']
    STARTTIME = ptb2ephys(exp['D'][i]['STARTCLOCKTIME'])
    STOPTIME = ptb2ephys(exp['D'][i]['ENDCLOCKTIME'])
    plt.axvspan(STARTTIME, STOPTIME, color=color, alpha=0.1)
...
selected_cids = V2_cids  # Choose V2 neurons for visualization
...
def _get_forage_starts_ptb(exp, include=(
    'ForagePregenRepeatingNoise', 'ForageProceduralNoise',
    'ForageDots', 'ForagePregenGabor'
)):
    starts_ptb = []
    for iT in range(len(exp['D'])):
        prname = exp['D'][iT]['PR'].get('name', None)
        if prname not in include:
            continue
        PR = exp['D'][iT]['PR']
        # Prefer NoiseHistory first (frame flip times in col 0)
        NH = PR.get('NoiseHistory', None)
        if NH is not None and NH.shape[0] > 0:
            starts_ptb.append(float(NH[0, 0]))
            continue
        # Fallback to ProbeHistory (some pregenerated protocols)
        PH = PR.get('ProbeHistory', None)
        if PH is not None and PH.shape[0] > 0:
            # Column 3 often stores PTB flip times
            starts_ptb.append(float(PH[0, 3]))
    if len(starts_ptb) == 0:
        return np.array([], dtype=float)
    arr = np.asarray(starts_ptb, dtype=float)
...
forage_ptb = _get_forage_starts_ptb(exp)
forage_ephys = ptb2ephys(forage_ptb) if forage_ptb.size > 0 else np.array([], dtype=float)
...
plot_saccade_heatmap(mat_v1, t_rel, f"Saccade-triggered response (V1), Nsacc={mat_v1.shape[0]}", sort_by='peak')
plot_saccade_heatmap(mat_v2, t_rel, f"Saccade-triggered response (V2), Nsacc={mat_v2.shape[0]}", sort_by='peak')

plot_saccade_heatmap(row_z(mat_v1), t_rel, f"Saccade-triggered response Z (V1), Nsacc={mat_v1.shape[0]}", vmin=-1.5, vmax=3.0, sort_by='peak')
plot_saccade_heatmap(row_z(mat_v2), t_rel, f"Saccade-triggered response Z (V2), Nsacc={mat_v2.shape[0]}", vmin=-1.5, vmax=3.0, sort_by='peak')
