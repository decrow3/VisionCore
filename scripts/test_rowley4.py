...
# --- Load unit depth report CSV ---
import pandas as pd
depth_csv_path = '/mnt/ssd2/RowleyMarmoV1V2/processed/Luke_2026-03-16/datasets/left_eye/gaborium_shallow_units_depth_report.csv'
if os.path.exists(depth_csv_path):
    depth_df = pd.read_csv(depth_csv_path)
    log_time('Loaded unit depth report CSV')
    print('Loaded unit depth report:', depth_csv_path)
    print(depth_df.head())
    # Plot depths by probe/shank if columns exist
    if {'unit_id','depth','shank'}.issubset(depth_df.columns):
        fig, ax = plt.subplots(figsize=(8,5))
        for shank, group in depth_df.groupby('shank'):
            ax.scatter(group['unit_id'], group['depth'], label=f'Shank {shank}', s=20)
        ax.set_xlabel('Unit ID')
        ax.set_ylabel('Depth (um)')
        ax.set_title('Unit Depths by Shank')
        ax.legend()
        fig.savefig(os.path.join(os.path.dirname(fpath), 'unit_depths_by_shank.pdf'))
        plt.close(fig)
    else:
        print('CSV missing required columns for plotting (unit_id, depth, shank)')
else:
    print('Depth report CSV not found:', depth_csv_path)
...
depth_threshold_um = 3820 - 1250.0
...
if use_rowley:
    ...
    sess = get_session('Luke', '2026-03-16')
    log_time('Created Rowley session handle')
...
# --- Select top 50 V1/V2 units by spike count from pre-selected candidates ---
...
print(f"Top 50 V1 cids (n={len(v1cids)}):", v1_orig)
print(f"Top 50 V2 cids (n={len(v2cids)}):", v2_orig)
...
log_time(f'Starting mean-rate plots for {len(cids)} units')
for cc in cids:
    fig, ax = plt.subplots()
    # ax.plot(time_bins[:-1], savgol_filter(np.nanmean(binned_spikes, 0)[:,cc], 10, 1))
    ax.plot(time_bins[:-1], np.nanmean(binned_spikes, 0)[:,cc])
...
log_time(f'Starting raster plots for {len(cids)} units')
for cc in cids:
