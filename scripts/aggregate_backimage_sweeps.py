...
RESULTS_DIR = Path('../declan/overnight_backimage_sweeps')
GLOB = 'sweep_*.pkl'
...
def stack_metric(results: list[dict], key: str) -> np.ndarray:
    ...
    arrs = [np.asarray(r[key], dtype=np.float32) for r in results]
...
def best_point_per_image(metric: str = 'i_spikes'):
    ...
    for r in results:
        ...
        if not np.isfinite(Z).any():
            continue
        flat = np.nanargmax(Z)
        i_sacc, i_eye = np.unravel_index(flat, Z.shape)
        rows.append({
            'image_file': r['image_file'],
            'best_value': float(Z[i_sacc, i_eye]),
