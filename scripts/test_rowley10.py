#%%
"""
test_rowley10.py — processed_mvp 03-02 loading test
...
2. Criterion 1 — Visual responsiveness via dots RF SNR (UPDATED)
    ...
    YAML remains a cross-check only, never an input.
    ...
    for session_dir in sorted(path for path in processed_root.iterdir() if path.is_dir()):
        for dataset_dir_candidate, eye_candidate in candidate_roots:
            ...
            try:
                ...
                print(f"[WARN] Skipping unexpected processed_mvp folder name: {session_dir.name}")
                continue
            configs.append({
...
single_session_requested = any(key in cli_args for key in ('subject', 'date', 'primary-eye', 'dataset-dir'))
...
if run_all_sessions and not _env_flag('ROWLEY_CHILD_RUN', False):
    session_configs = _discover_session_configs(legacy_processed_root, fix_name)
    if session_filter:
        session_configs = [cfg for cfg in session_configs if session_filter in f"{cfg['subject']}_{cfg['date']}"]
    print(f"Discovered {len(session_configs)} runnable processed_mvp dataset entries")
    failures = []
    for cfg in session_configs:
        session_label = f"{cfg['subject']}_{cfg['date']}"
        print("\n" + "=" * 80)
        print(f"Dispatching {session_label} | dataset_dir={cfg['dataset_dir']} | primary_eye={cfg['primary_eye']}")
        print("=" * 80)
        ...
        if result.returncode != 0:
            failures.append((session_label, cfg['dataset_dir'], result.returncode))
            print(f"[FAIL] {session_label} | {cfg['dataset_dir']} exited with code {result.returncode}")
    print("\n" + "=" * 80)
    print(f"Processed {len(session_configs)} dataset entries")
    if failures:
        print(f"Failures: {len(failures)}")
        for session_label, dataset_dir_failed, code in failures:
            print(f"  {session_label} | {dataset_dir_failed} -> exit {code}")

    # ── Cross-session summary figure ──────────────────────────────────────────
    print("\nBuilding cross-session summary figure...")
    mcfarland_dir = FIGURES_DIR / 'mcfarland'
    pkl_paths = sorted(mcfarland_dir.glob('*_v10/cyclopean-dpi/mcfarland_fixrsvp_*_default.pkl'))
    if not pkl_paths:
        ...
        for pkl_path in pkl_paths:
            ...
            sess_lbl  = pkl_path.parent.parent.name.replace('_v10', '')
        ...
        if not _cs_sessions:
            ...
            fig_cs.suptitle('Cross-session covariance summary (v10, cyclopean DPI)', fontsize=11)
            fig_cs.tight_layout(rect=(0, 0, 1, 0.97))
            cs_path = mcfarland_dir / 'cross_session_summary_v10.pdf'
    ...
    all_pkl_paths = sorted(mcfarland_dir.glob('*_v10/*/mcfarland_fixrsvp_*.pkl'))
    ...
    if _subdirs_ordered:
        ...
        for _si, _subdir in enumerate(_subdirs_ordered):
            ...
            for _pkl_p in sorted(_subdir_pkls[_subdir]):
                ...
                _slbl = _pkl_p.parent.parent.name.replace('_v10', '')
        ...
        fig_comb.suptitle('FEM fraction — combined histogram by eye trace type (v10, 20 ms window)',
                          fontsize=11)
        fig_comb.tight_layout(rect=(0, 0, 1, 0.97))
        comb_path = mcfarland_dir / 'cross_session_fem_combined_v10.pdf'
...
dots_binned_path = (legacy_processed_root / f'{subject}_{date}'
                    / 'dpi_calibration' / 'dots_binned_data.dset')
dots_primary_eye_dir = (legacy_processed_root / f'{subject}_{date}'
                        / 'dpi_calibration' / f'{primary_eye}_eye')
...
BASE_FIGURES_DIR = FIGURES_DIR / 'mcfarland' / f'{subject}_{date}_v10'
...
visual_mask = (max_snr_visual >= snr_threshold_primary) if visual_snr_available else np.ones(n_all_units, dtype=bool)
if visual_snr_available:
...
for subdir_name, eye_source in eye_configs:
    ...
    output = {
        ...
        'meta': {
            ...
            'estimator':    'VisionCore.covariance v10',
        },
    }
...
if _fem_summaries:
    ...
    fig_sum.suptitle(f'Covariance decomposition summary | {session_label} (v10)')
    ...
    fig_fem.suptitle(f'FEM fraction comparison | {session_label} (v10)', fontsize=11)
...
with open(txt_path, 'w') as f:
    f.write(f"Covariance decomposition summary — v10\n")
    ...
    f.write(f"  visual_snr_mode=dots_rf  visual_snr_source={visual_snr_source}\n")
