...
session for an independent audit. Running this fig4-aligned version does NOT
reproduce fig2 (obs_pipeline median ~0.476 vs fig2 ~0.725 on session 0), so it
is confounded: the fig4 alignment (min_total_spikes=200, fixation<1deg, its own
...
Usage:
    uv run ryan/fig4/simulate_fig4d_control.py [--n-sim 5]
"""
import argparse
import numpy as np
import dill
...
def main():
    ...
    if "eyepos_used" not in data["session_results"][0]:
        raise RuntimeError(
            "Cache lacks eyepos_used. Regenerate via load_fig4_data(recompute=True)."
        )
